...
export abstract class BaseAccountAPI {
  ...
  abstract encodeCustom (target: string, functionFragment: string, data: any[]): Promise<string>
  ...
  async encodeUserOpCallDataAndGasLimitCustom (detailsForUserOp: TransactionDetailsForUserOp): Promise<{ callData: string, callGasLimit: BigNumber }> {
    function parseNumber (a: any): BigNumber | null {
      if (a == null || a === '') return null
      return BigNumber.from(a.toString())
    }

    const callData = await this.encodeCustom(detailsForUserOp.target, 'paySubscription', detailsForUserOp.data)
   ...
   * @param info
   */
  async createUnsignedUserOp (info: TransactionDetailsForUserOp, custom?: boolean): Promise<UserOperationStruct> {
    const {
      callData,
      callGasLimit
    } = ((custom === true) ? await this.encodeUserOpCallDataAndGasLimitCustom(info) : await this.encodeUserOpCallDataAndGasLimit(info))
  ...
  async createSignedUserOpCustom (info: TransactionDetailsForUserOp, custom?: boolean): Promise<UserOperationStruct> {
    return await this.signUserOp(await this.createUnsignedUserOp(info, custom))
  }
