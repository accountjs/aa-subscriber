  ...
  async handleMethod (method: string, params: any[]): Promise<any> {
    ...
    switch (method) {
      ...
      case 'eth_sendUserOperation':
        ...
        break
      case 'qaa_setSubscription':
        result = await this.methodHandler.addSubscription(params[0], params[1], params[2], params[3], params[4])
        // address, id, starttime, interval, endtime (paySubscription(id))
        break
      case 'qaa_delSubscription':
        result = await this.methodHandler.removeSubscription(params[0], params[1])
        // address, id
        break
